...
External hooks:
  - ``ctx.select_feature(feat)``  — clicked when the user picks a row
    from the top-features table.
  - ``ctx.state``  — read for the active dataset's images, dim, etc.
"""
...
def build(ctx, on_feature_pick: Optional[Callable[[int], None]] = None):
    ...
    patch_grid).
    """
    pg = ctx.state.patch_grid

    rows, cols, xs, ys = _patch_grid_arrays(pg)
    patch_grid_source = ColumnDataSource(data=dict(x=xs, y=ys, row=rows, col=cols))
    # All columns share one length (0 until an image loads). Mixing an empty
    # `image` with length-1 coord columns trips Bokeh's CDS length validation
    # (a BokehUserWarning on 3.8, stricter on later versions); the image glyph
    # needs no placeholder row to position the (absent) image.
    ...
    def _do_load_image():
        ...
        ctx.ui.current_patch_img = img_idx
        try:
            pil = ctx.load_image(img_idx)
        ...
        try:
            z_np = compute_patch_activations(ctx, img_idx)
            ctx.ui.current_patch_z = z_np
        except Exception as e:
    ...
    def _top_features_for(patch_indices, top_n: int = 50):
        ...
        Pure-ish: reads ``ctx.ui.current_patch_z`` and
        ``ctx.state.feature_frequency`` / ``feature_mean_act`` only.
        """
        z_np = ctx.ui.current_patch_z
        if z_np is None:
            return [], [], [], []
        state = ctx.state
    ...
    def _on_patch_select(attr, old, new):
        if ctx.ui.current_patch_img is None:
            return
    ...
    def on_dataset_changed(idx: int) -> None:
        rebuild_grid(ctx.state.patch_grid)
