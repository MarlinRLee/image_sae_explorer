...
comparison datasets are loaded lazily on first use via ``ctx.ensure_loaded``.
The "SAE A" inputs are auto-filled by ``ctx.select_feature`` (via the
bootstrap) so the user can compare the currently-selected feature without
re-typing.

Tests can call ``build(ctx, datasets=[fake_ds_a, fake_ds_b])`` and inspect
the returned widgets without driving Bokeh's event loop.
"""
...
def build(ctx, datasets: list,
    ...
    Args:
        ...
        datasets: the lazy-loaded dataset list (typically ``ctx.datasets``).
        ensure_loaded: callable that lazily-loads dataset[idx] on first
            access. Defaults to ``ctx.ensure_loaded`` if None.
    ...
    def _generate(force: bool = False):
        ...
        try:
            ...
            loader = ensure_loaded or ctx.ensure_loaded
    ...
    # value_throttled (not value): re-render the existing comparison only once
    # when the user releases the opacity handle, not on every intermediate
    # drag tick (each tick re-encodes 8 images).
    cmp_alpha_slider.on_change('value_throttled', lambda attr, old, new: _generate())
