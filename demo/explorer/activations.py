...
def _load_inference_pieces(ctx, ds: dict):
    ...
    if not sae_file:
        ...
        return None
    sae_path = os.path.join(ctx.args.data_dir, sae_file)
...
def compute_patch_activations(ctx, img_idx: int):
    """Return (n_patches, d_sae) float32 for the active dataset, or None.

    Reads from the session ``ctx.state`` and ``ctx.args`` (for the LRU
    cache size limit). The cache is keyed per-dataset (lives on the
    ...
    if z_np is None:
        z_np = _run_inference(ctx, img_idx, ds)
