...
bootstrap can place the widgets in the layout and reuse the click handler
for the auto-Gemini-on-visit flow.
...
def build(ctx,
          get_current_feat: Callable[[], Optional[int]],
          on_label_applied: Callable[[int, str], None],
          api_key: Optional[str] = None,
          model: str = DEFAULT_MODEL):
    """Build the Gemini button + status div and return ``(btn, status_div, on_click)``.

    ``ctx``  the session :class:`~explorer.context.Context`; read for the
        active ``state`` and the default API key.
    ...
    ``on_label_applied(feat, label)``  called from the doc thread once
        ...
        detail panel.
    ``api_key``  if None, defaults to ``ctx.args.google_api_key`` /
    ...
    """
    if api_key is None:
        api_key = (getattr(ctx.args, 'google_api_key', None)
