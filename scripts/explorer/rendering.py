...
every feature click and every UMAP hover. Each takes the session
:class:`~explorer.context.Context` explicitly (for ``state`` / ``ui`` /
``args`` and ``load_image``), so they're safe to call from the render
worker threads.
...
The per-session hover-thumbnail cache lives on ``ctx.ui.umap_mei_cache``
(not a module global) so two sessions on different datasets can't read each
other's thumbnails. Every helper here takes the session ``ctx`` explicitly.
"""
...
def render_heatmap_overlay(ctx, img_idx: int, heatmap_grid, size: int,
    ...
    full_img = ctx.load_image(img_idx).resize(
    ...
    if zoom_patches is None or zoom_patches >= pg:
        ...
        # Keep the crop window exactly `zoom_patches` patches wide at every
        # step. Center it on the peak, then SLIDE it fully inside the image
        # rather than clamping (which would shrink the window for off-centre
        # peaks and make the first step down from "full" jump ~2x).
        win = min(int(zoom_patches) * patch_px, state.image_size)
        cy = peak_row * patch_px + patch_px // 2
        cx = peak_col * patch_px + patch_px // 2
        x0 = min(max(cx - win // 2, 0), state.image_size - win)
        y0 = min(max(cy - win // 2, 0), state.image_size - win)
        x1 = x0 + win
        y1 = y0 + win
...
def _mei_data_urls(ctx, feat: int, n: int = UMAP_THUMBS_PER_FEAT, hm_tensor=None):
    ...
    for slot in range(n):
        try:
            ...
            if img_i < 0:
                continue
            img = ctx.load_image(img_i).resize(
...
def _prewarm_hover_cache_async(ctx) -> None:
    """Background-fill ``ctx.ui.umap_mei_cache`` for the top-K most-frequent
    live features so the first hover doesn't stall.

    K is read from ``ctx.args.hover_prewarm_count``. The caller
    (dataset-switch path) is responsible for clearing the cache before
    invoking this. A token bump (``ctx.ui.prewarm_token``) cancels any
    in-flight prewarm so cache entries can never come from a stale
    dataset's tensors.
    """
    args  = ctx.args
    state = ctx.state
    ui    = ctx.ui
    cache = ui.umap_mei_cache
