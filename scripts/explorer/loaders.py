...
def load_dataset(path: str,
    ...
    entry = {
        ...
        'crop_img_idx':               d.get('crop_img_idx', d['top_img_idx']),
        'crop_img_act':               d.get('crop_img_act', d['top_img_act']),
        ...
        'interp_index':               d.get('interp_index', None),
...
def ensure_loaded(datasets: List[dict], idx: int) -> None:
    ...
    The build bakes every model's sidecars into the image, so the ``.pt`` is
    always on disk by the time it is selected — this just defers the
    ``torch.load`` cost until the compare is first opened.
    """
    ds = datasets[idx]
