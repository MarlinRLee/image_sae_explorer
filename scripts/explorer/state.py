...
class _UI:
    ...
    umap_hover_pending = None          # active hover-debounce timeout handle
    umap_hover_xy = None               # latest (x, y) cursor coords awaiting resolve
    ...
    def __init__(self):
        # feat_idx -> list of up-to-3 hover-thumbnail data URLs. Per session
        # (was a module global shared across sessions in rendering.py) so two
        # sessions on different datasets can't read each other's thumbnails.
        # Cleared by the dataset-switch / UMAP-type-toggle paths.
        self.umap_mei_cache: dict = {}
...
class _State:
    ...
    # Dataset-dict keys copied verbatim onto the state as attributes of the
    # same name. Required: absent in ds -> KeyError on apply (a precompute
    ...
    _REQUIRED = (
        ...
        'feature_names', 'auto_interp_names', 'feature_name_authors',
        'auto_interp_authors', 'feature_names_history', 'auto_interp_history',
        'names_file', 'auto_interp_file', 'feature_name_authors_file',
        'auto_interp_authors_file', 'feature_names_history_file',
        'auto_interp_history_file',
    )
    # Optional keys (set to None when missing). Heatmaps and CLIP are absent
    # on some precompute outputs (e.g. CLS-token SAEs, no-clip sidecar).
    _OPTIONAL = (
        'top_heatmaps', 'mean_heatmaps', 'crop_heatmaps',
        'clip_scores', 'clip_vocab', 'clip_embeds', 'clip_scores_f32',
        'interp_index',
    )
    ...
    def apply(self, idx: int) -> None:
        ...
        # Per-feature Interpretability Index (Klindt et al., 2023): higher =
        # more coherent top-MEIs. Stored as -LPIPS in [-1, 0]; we report it on
        # a 0..1 coherence scale (1 + stored = 1 - LPIPS) so higher is better
        # and the range is intuitive. Absent on older precompute outputs.
        self.ii       = (1.0 + self.interp_index.numpy()
                         if self.interp_index is not None else None)
        self.has_ii   = self.ii is not None
