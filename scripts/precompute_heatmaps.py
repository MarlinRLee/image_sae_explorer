...
Contents of the sidecar:
    ...
    crop_heatmaps  (d_model, n_crop, patch_grid²)  float16
...
def build_inverted_index(top_idx, mean_idx, crop_idx, p75_idx):
    """
    ...
    for f in range(top_idx.shape[0]):
        ...
        for j in range(crop_idx.shape[1]):
            i = int(crop_idx[f, j].item())
            if i >= 0:
                img_to_entries[i].append(('crop', f, j))
...
def main():
    ...
    crop_idx = data.get('crop_img_idx', data['top_img_idx'])
    p75_idx  = data['p75_img_idx']

    n_top  = top_idx.shape[1]
    n_mean = mean_idx.shape[1]
    n_crop = crop_idx.shape[1]
    ...
    print(f"  Image slots per feature: top={n_top}, mean={n_mean}, "
          f"crop={n_crop}, p75={n_p75}")

    # --- Build inverted index ---
    print("Building inverted index...")
    img_to_entries = build_inverted_index(top_idx, mean_idx, crop_idx, p75_idx)
    ...
    crop_heatmaps = torch.zeros(d_model, n_crop, n_sq, dtype=torch.float16)
    p75_heatmaps  = torch.zeros(d_model, n_p75,  n_sq, dtype=torch.float16)
    est_mb = (top_heatmaps.numel() + mean_heatmaps.numel()
              + crop_heatmaps.numel() + p75_heatmaps.numel()) * 2 / 1e6
            ...
            for local_pos, global_img_i in valid_in_batch:
                ...
                for (set_name, feat, slot_j) in img_to_entries[global_img_i]:
                    ...
                    if set_name == 'top':
                        ...
                        crop_heatmaps[feat, slot_j] = torch.from_numpy(hmap_vec)
                    else:  # 'p75'
    ...
    print(f"  crop_heatmaps : {tuple(crop_heatmaps.shape)}")
