...
from explorer.hf_assets import download_model_assets


def fetch_models(reg: Registry, target: str, token: str | None) -> None:
    """Download .pt + _heatmaps.pt + .pth for every model in the registry."""
    ...
    for entry in reg.all_entries:
        print(f"[{entry.label}]")
        download_model_assets(reg.hf_data_repo, entry.data_file,
                              entry.sae_file, target, token, label=entry.label)
...
def launch_bokeh(registry_path: str, target: str, image_dir: str,
    ...
    cmd = [
        ...
        '--data-dir',  target,
        '--image-dir', image_dir,
        # The hover-thumbnail prewarm runs once per Bokeh session (the app
        # script re-executes on every browser connection) and renders
        # ``hover_prewarm_count`` features × 3 thumbnails through a 6-worker
        # pool. On the 2-vCPU HF Space that storm starves Tornado's single
        # event loop, so the HF proxy times out and 500s unrelated requests
        # (including bokeh.min.js → the app never boots). Disable it here;
        # hover thumbnails still render lazily on first hover.
        '--hover-prewarm-count', '0',
    ]
    # Serve the ~1.3 MB BokehJS bundles from the Bokeh CDN instead of through
    # this container's /static handler. Behind the HF proxy the single-process
    # Tornado server can't reliably serve those large files — requests for
    # bokeh.min.js intermittently 500, and when the *core* bundle is the one
    # that fails the plugins throw "Cannot find Bokeh x.y.z" and the page
    # renders blank. The CDN is version-pinned to the installed bokeh, so it
    # always matches what the server serialized. ``setdefault`` lets a deploy
    # override it (e.g. BOKEH_RESOURCES=server) without editing this file.
    os.environ.setdefault('BOKEH_RESOURCES', 'cdn')
    print(f"[launch] BOKEH_RESOURCES={os.environ['BOKEH_RESOURCES']} {' '.join(cmd)}")
    os.execvp(cmd[0], cmd)
