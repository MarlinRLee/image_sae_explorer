...
def _make_explorer_pt(image_paths,
    ...
    entry = {
        ...
        'crop_img_idx':      torch.from_numpy(crop_idx),
        'crop_img_act':      torch.from_numpy(crop_act),
...
def _make_heatmap_sidecar(d_model: int, n_top_slots: int, patch_grid: int, seed: int = 0):
    ...
    return {
        ...
        'crop_heatmaps': torch.from_numpy(rng.random(shape).astype(np.float16)),
