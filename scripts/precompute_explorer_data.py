...
Processes images through DINOv3 → SAE, then computes:
  ...
  - Per-feature crop-averaged images (mean of each image's top-K positive
    patches — "crop and average", a middle ground between max and mean)
...
# Number of top patches averaged per image for the "crop and average" ranking.
# K=8 is the common literature default for ~256-patch ViT grids: localized
# enough to ignore the global background, robust enough not to hinge on a
# single outlier patch (the max ranking already covers k=1).
CROP_K = 8
...
class FeatureImageTracker:
    ...
    def __init__(self, n_features, top_n=16):
        ...
        # Parallel heaps ranked by the "crop and average" score: the mean of
        # each image's top-CROP_K positive patches (see CROP_K).
        self.crop_heaps          = [[] for _ in range(n_features)]
        self.crop_heap_min_vals  = np.full(n_features, -np.inf, dtype=np.float32)
        self.crop_heap_sizes     = np.zeros(n_features, dtype=np.int32)
    ...
    def update_crop_heaps(self, image_idx, crop_acts):
        self._update_heap_set(
            self.crop_heaps, self.crop_heap_min_vals, self.crop_heap_sizes,
            image_idx, crop_acts,
        )
    ...
    def finalize(self, n_top, umap_data, umap_img_idx, n_p75=8):
        ...
        crop_img_idx, crop_img_act = self._heaps_to_tensors(self.crop_heaps, n_top)
        self.crop_heaps = None
...
def main():
    ...
    # --- Load patch-norm stats (optional) ---
    patch_norm_mean = None
    patch_norm_std  = None
    if args.patch_norm_stats:
        print(f"Loading patch-norm stats from {args.patch_norm_stats}...")
        _stats = torch.load(args.patch_norm_stats, map_location=device, weights_only=True)
        patch_norm_mean = _stats['positional_mean'].float().to(device)  # (n_patches, d_hidden)
        patch_norm_std  = _stats['std'].float().to(device)              # (d_hidden,)
        print(f"  positional_mean: {tuple(patch_norm_mean.shape)}, std: {tuple(patch_norm_std.shape)}")
    ...
    with torch.inference_mode():
        for batch_i, (imgs, paths) in enumerate(loader):
            ...
            # "Crop and average": per image+feature, the mean of the top-K
            # patches restricted to positive activations. Take the K largest
            # patch values (a partial sort along the patch axis), then average
            # only those that are > 0 — so a feature firing on fewer than K
            # patches isn't diluted by zeros, and one firing on more than K
            # contributes its K strongest patches. Reduces to mean-of-positives
            # when n_patches <= K. Features with no positive patch score 0.
            k = min(CROP_K, n_patches)
            topk_patches = np.partition(z_batch_cpu, n_patches - k, axis=1)[:, n_patches - k:, :]
            topk_pos      = np.maximum(topk_patches, 0.0)     # (bs, k, d_model)
            topk_pos_cnt  = (topk_patches > 0).sum(axis=1)    # (bs, d_model)
            crop_acts_batch = np.where(
                topk_pos_cnt > 0,
                topk_pos.sum(axis=1) / np.maximum(topk_pos_cnt, 1),
                0.0,
            ).astype(np.float32)
            ...
            for i in range(bs):
                ...
                tracker.update_crop_heaps(img_idx, crop_acts_batch[i])
